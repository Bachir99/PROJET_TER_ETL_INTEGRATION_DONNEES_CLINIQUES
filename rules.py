...
def V_date_of_birth1(df, column_name):
    today = datetime.now()
...
def V_dateOfDeath(df, column_name):
...
def V_NotNull1(row,column_name):
        ...
        for function_name in functions:
            if function_name in validation_functions:
                ...
                if function_name == "D_patientDeceased":
                    ...
                    df = df[df.apply(lambda row: function(row, column), axis=1)]
                else:
                    df = function(df, column)
